import { LAMPORTS_PER_SOL, Transaction, SystemProgram, Keypair, Connection, PublicKey } from "@solana/web3.js";
...
const MY_TOKEN_METADATA: UploadMetadataInput = {
    ...
    image: process.env.IMAGE_URL || '' //add public URL to image you'd like to use
}
...
const main = async() => {
    ...
    const publicKeyString = process.env.ACCOUNT_PUBLIC_KEY || '';
    const publicKey = new PublicKey(publicKeyString);
    const publicKeyArray = Uint8Array.from(publicKey.toBuffer());
    const userWallet = Keypair.fromSeed(publicKeyArray);

    // Airdropping some amount in the wallet so that it has some 'prior credit'
